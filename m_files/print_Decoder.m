...
fprintf (fid, "always @ (posedge clk or negedge rst) begin\n");
    ...
    fprintf (fid, "\telse begin\n");
        ...
        %case of "cnt".
        fprintf (fid, "\t\tcase (cnt)\n");
            fprintf (fid, "\t\t\t8'd0: begin//reading part.\n");
                fprintf (fid, "\t\t\t\tif (in_valid == 0)\n");
                fprintf (fid, "\t\t\t\tcnt <= 8'd1;\n");
            fprintf (fid, "\t\t\tend\n");

            fprintf (fid, "\t\t\t8'd1: cnt <= 8'd2;//VNs' reading.\n");
            for cnt = 2 : 9
                fprintf (fid, "\t\t\t8'd" + string(cnt) + ": cnt <= 8'd" + string(cnt + 1) + ";//CNU.\n");
            end
            for cnt = 10 : 14
                fprintf (fid, "\t\t\t8'd" + string(cnt) + ": cnt <= 8'd" + string(cnt + 1) + ";//VNU.\n");
            end

            fprintf (fid, "\t\t\t8'd15: begin\n");
                for row = 1 : m
                    fprintf (fid, "\t\t\t\tCheck_" + string (row) + " <= ");
                    index = find (H(row, :));
                ...
                fprintf (fid, "\t\t\t\tcnt <= 8'd16;");
            fprintf (fid, "\t\t\tend\n");
            
            fprintf (fid, "\t\t\t8'd16: begin\n");
                fprintf (fid, "\t\t\t\tCheck_Sum <= ");
                for row = 1 : m - 1
                    fprintf (fid, "Check_" + string(row) + " | ");
                end
                fprintf (fid, "Check_" + string(m) + ";\n");
                fprintf (fid, "\t\t\t\tcnt <= 8'd17;\n");
            fprintf (fid, "\t\t\tend\n");

            fprintf (fid, "\t\t\t8'd17: begin\n");
                fprintf (fid, "\t\t\t\tif (Check_Sum == 0 || iter == 5'd30) begin\n");
                fprintf (fid, "\t\t\t\t\tcnt <= 8'd18;\n");
                fprintf (fid, "\t\t\t\t\tout_valid <= 1;\n");
                fprintf (fid, "\t\t\t\tend\n");
                fprintf (fid, "\t\t\t\telse begin\n");
                fprintf (fid, "\t\t\t\t\tout_valid <= 0;\n");
                fprintf (fid, "\t\t\t\t\tcnt <= 8'd2;\n");
                fprintf (fid, "\t\t\t\t\titer <= iter + 1;\n");
                fprintf (fid, "\t\t\t\tend\n");
            fprintf (fid, "\t\t\tend\n");

            fprintf (fid, "\t\t\t8'd18: begin\n");
            ...
            fprintf (fid, "\t\t\tdefault: begin//8'd19 : don't work.\n");
                fprintf (fid, "\t\t\t\tif (in_valid == 1)\n");
                fprintf (fid, "\t\t\t\tcnt <= 8'd0;\n");
            fprintf (fid, "\t\t\tend\n");
        fprintf (fid, "\t\tendcase\n");
    fprintf (fid, "\tend\n");
fprintf (fid, "end\n");
